...
void editorDelChar() {
    // If the cursor is past the end of the file, there is nothing to delete. Return immediately.
...
void editorScroll() {
    ...
    // Checks if cursor is above visible window. If so, scrolls up to where cursor is.
    if (E.cy < E.rowoff) {
        E.rowoff = E.cy;
    }
    // Checks if cursor is past bottom of visible window
...
void editorMoveCursor(int key) {
    ...
    switch (key) {
        case ARROW_LEFT:
            if (E.cx != 0) {
                E.cx--;
            } else if (E.cy > 0) {  //Ensures user is not on the first line before moving them up
                E.cy--;
...
void editorProcessKeypress() {
    ...
    // Resets quit_times back to 3 at the end of function
...
void initEditor() {
    ...
    // Scrolls to top of screen by default
    E.rowoff = 0;
